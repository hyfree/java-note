...
public class IndexController {
	...
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response) {
		...
		List<NoteinfoWithBLOBs> list = noteService.findTwenty(user.getId().toString());
		// 生成Html文件
		// 判断是否有文件夹 没有就生成
	...
	public List<String> readFile(File file) {
		...
		if (fs!=null&&fs.length > 0){
			for (File f : fs) {
				// System.out.println(f.isDirectory() ? "文件夹：" +
				// f.getAbsolutePath()
				// : "文件：" + f.getName());
				list.add(f.getName());
				if (f.isDirectory()) {
	...
	public void html(List<String> fileList, List<NoteinfoWithBLOBs> dataList, HttpServletRequest request,
			HttpServletResponse response) {
//		NoteInfoWithBLOBs notes = new NoteInfoWithBLOBs();
		
		if (fileList.size() > 0) {
			for (NoteinfoWithBLOBs note : dataList) {
